...
{
    public partial class queryOutboxDocReceivedDate : Form
    {
        ...
        private void queryOutboxDocReceivedDate_Load(object sender, EventArgs e)
        {
            // Form_Load içinde:
        ...
        public queryOutboxDocReceivedDate()
        {
            InitializeComponent();
        }

        private void btnQueryOutboxDocReceivedDate_Click(object sender, EventArgs e)
        {
            ...
            using (var scope = new OperationContextScope(client.InnerChannel))
            {
                ...
                string startDate = dateTimeStart.Value.ToString("yyyy-MM-dd HH:mm:ss.fff");
                string finishDate = dateTimeEnd.Value.ToString("yyyy-MM-dd HH:mm:ss.fff");
                string minRecordId = textBoxMinRecordId.Text.Trim();

                var outdocreceivedate = client.QueryOutboxDocumentsWithReceivedDate(startDate, finishDate, documentType, queried, withXML, minRecordId);

                try
                {

                    string mesaj = "Hata durumu: " + outdocreceivedate.queryState.ToString() + Environment.NewLine +
                        "Açıklama: " + outdocreceivedate.stateExplanation + Environment.NewLine +
                        "Belge sayısı: " + outdocreceivedate.documentsCount;
                    ...
                    switch (withXML)
                    {
                        case "XML":
                            ...
                            for (int i = 0; i < outdocreceivedate.documents.Length; i++)
                            {
                                var doc = outdocreceivedate.documents[i];
